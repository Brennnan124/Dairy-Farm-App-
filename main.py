...
def main():
    ...
    try:
        ...
        all_milk = pd.DataFrame()  # Fallback to empty DataFrame if loading fails
        st.warning(f"Error loading milk production data: {e}")

    try:
        all_feeds_recv = to_date(load_table("feeds_received"), "date")
    except Exception as e:
        all_feeds_recv = pd.DataFrame()
        st.warning(f"Error loading feeds received data: {e}")

    try:
        all_feeds_used = to_date(load_table("feeds_used"), "date")
    except Exception as e:
        all_feeds_used = pd.DataFrame()
        st.warning(f"Error loading feeds used data: {e}")

    try:
        all_cows = load_table("cows")
    except Exception as e:
        all_cows = pd.DataFrame()
        st.warning(f"Error loading cows data: {e}")

    try:
        all_obs = to_date(load_table("observations"), "date")
    except Exception as e:
        all_obs = pd.DataFrame()
        st.warning(f"Error loading observations data: {e}")

    # Calculate min and max dates with fallback
    min_date = min([d for d in [all_milk["date"].min() if not all_milk.empty and "date" in all_milk.columns else None,
                                all_feeds_recv["date"].min() if not all_feeds_recv.empty and "date" in all_feeds_recv.columns else None,
                                all_feeds_used["date"].min() if not all_feeds_used.empty and "date" in all_feeds_used.columns else None,
                                all_obs["date"].min() if not all_obs.empty and "date" in all_obs.columns else None] if d is not None], default=date.today())
    max_date = max([d for d in [all_milk["date"].max() if not all_milk.empty and "date" in all_milk.columns else None,
                                all_feeds_recv["date"].max() if not all_feeds_recv.empty and "date" in all_feeds_recv.columns else None,
                                all_feeds_used["date"].max() if not all_feeds_used.empty and "date" in all_feeds_used.columns else None,
                                all_obs["date"].max() if not all_obs.empty and "date" in all_obs.columns else None] if d is not None], default=date.today())

    if role == "Manager" and page == "Reports":
        with st.sidebar:
            st.markdown("### Analysis Filters")
            date_range = st.date_input("Date Range", value=(min_date, max_date if max_date > min_date else min_date), min_value=min_date, max_value=max_date, key="date_range")
            start_date = date_range[0] if isinstance(date_range, tuple) and len(date_range) == 2 else min_date
            end_date = date_range[1] if isinstance(date_range, tuple) and len(date_range) == 2 else max_date
