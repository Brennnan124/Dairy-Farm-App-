...
def milk_records_page(username):
    ...
    with col1:
        ...
        litres_sell = st.number_input("Litres for Sale", min_value=0.0, max_value=100000.0, step=0.1, value=0.0, key="milk_sell")
        record_date = st.date_input("Date of Recording", value=date.today(), key="milk_date")
    with col2:
        litres_calves = st.number_input("Litres for Calves", min_value=0.0, max_value=100000.0, step=0.1, value=0.0, key="milk_calves")
